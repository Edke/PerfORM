...
class ANSI_UTF8_output
{
	var $last_class;


	function classToMarker($class)
	{
	    switch ($class)
	    {
		case 'sql-comment':
...
//              $word = htmlEntities($word, ENT_COMPAT, 'UTF-8');
		...
		else
		{
			if($this->last_class == null)
			{
				...
				return $this->classToMarker($class).$word;
			}
			$this->last_class = $class;
			if($class == null) return "%n$word";
			return "%n".$this->classToMarker($class).$word;
		}
	}
